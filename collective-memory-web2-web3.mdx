---
title: "Collective Memory web2-web3"
---

## -- title: "Web2 ↔ Web3 Architecture" description: "How Collective Memory bridges Firebase with an immutable on-chain ledger on Base"
...
| Repo | Purpose | Status |
| --- | --- | --- |
| [`cm-minimal-contracts`](https://github.com/collective-memory-dev/cm-minimal-contracts) | Production ledger router, vaults, tokens (Hardhat \+ OZ Upgrades) | **Active — Base mainnet** |
| [`cm-contracts`](https://github.com/collective-memory-dev/cm-contracts) | ATTNToken, ATTNTreasury, BeliefVault/VaultFactory, gasless treasury (Foundry) | Base Sepolia |
| [`contracts`](https://github.com/collective-memory-dev/contracts) | Original Uniswap V2 AMM prototype (Hardhat) | Legacy / reference |
| [`cm-genlayer-example`](https://github.com/collective-memory-dev/cm-genlayer-example) | GenLayer integration POC | Experimental |
| [`genlayer-poc`](https://github.com/collective-memory-dev/genlayer-poc) | GenLayer proof of concept | Experimental |
| [`app-firebase-functions`](https://github.com/collective-memory-dev/app-firebase-functions) | Firebase Cloud Functions — web2 backend driving the pipeline | **Active** |
...
<AccordionGroup>
  <Accordion title="Gasless UX">
    Users never interact with the blockchain directly. Backend operator wallets pay all gas. Users don't need wallets or ETH — the entire crypto layer is invisible to them.
  </Accordion>

  <Accordion title="Deterministic Deployment (CREATE2)">
    Vault and token addresses are known before deployment. The backend can pre-compute addresses and reference them in Firestore without waiting for on-chain confirmation.
  </Accordion>

  <Accordion title="Beacon Proxies">
    A single upgrade to a beacon implementation upgrades thousands of vaults or tokens simultaneously — critical for a system that creates a new vault and token per memory.
  </Accordion>

  <Accordion title="Treasury as Backstop">
    All pATTN originates from the Treasury. When a Memory Vault doesn't have enough pATTN to pay a redemption (because some was already paid to the creator), the Treasury covers the difference.
  </Accordion>

  <Accordion title="Memo Strings for Audit Trail">
    Every on-chain movement carries a `memo` parameter that links back to the web2 transaction ID, enabling bidirectional audit trails between Firestore and the blockchain.
  </Accordion>

  <Accordion title="Managed AMM">
    Moving the bonding curve math off-chain retains the original Uniswap-inspired economic model while dramatically reducing gas costs and enabling batch settlement.
  </Accordion>
</AccordionGroup>